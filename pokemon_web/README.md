...
- tests/: queries.py 조회 결과 검증용 pytest 테스트
...
- 이미지는 파일로 저장하지 않고, 이미지URL만 CSV에 저장해 표시할 때 원본(img.gamewith.jp)을 직접 불러옵니다.
- pokemon_db.csv 의 이미지URL 컬럼(영어이름 다음), items_db.csv 의 이미지URL 컬럼을 사용합니다.
- 목록/상세 모두 CSS 로 크기를 고정(sprite 40px, sprite-lg 96px, object-fit:contain)해 표가 뒤틀리지 않습니다.
- 이미지 파일명은 전국도감 번호 기준이며 폼(메가/리전/성별 등)은 접미사(예: 006_1)를 사용합니다.

## 페이지

- /pokemon : 포켓몬 복합 검색 목록
  - 이름 검색, 타입 1~2개(AND/OR), 특성, 배우는 기술 최대 4개(AND/OR),
    내성 타입 여러 개(AND/OR), 메가진화 포함/제외
  - 정렬: 도감번호/종족값/각 스탯/순위/이름 중 최대 3개를 우선순위대로 지정 (오름/내림)
  - 도감번호: 전국도감 기준 번호(INTEGER). 폼(메가 등)은 같은 종끼리 같은 번호를 공유하므로 기본키가 아닌 속성 컬럼입니다.
- /pokemon/{id} : 상세 (특성·타입상성(약점/반감/무효)·배우는 기술 조인, 종족값 막대 그래프)
- /moves, /moves/{id} : 기술 목록/상세 (이름·효과 검색, 타입/분류 필터, 다중 정렬, 배우는 포켓몬 조인)
- /abilities, /abilities/{id} : 특성 목록/상세 (이름·효과 검색, 보유 포켓몬 조인)
- /items, /items/{id} : 지닌물건 목록/상세 (이름·효과 검색, 이미지·입수방법·원본 링크)
- /types : 타입 상성표 매트릭스 (범례, 마우스 오버 시 행/열 강조)
- 존재하지 않는 상세 ID로 접근하면 base 레이아웃을 사용한 404 페이지를 보여줍니다

## 화면 공통 보강 사항

- 오류 페이지 통일: 존재하지 않는 경로(404), 잘못된 파라미터 형식(400, 예: /pokemon/abc),
  서버 오류(500) 모두 JSON 대신 base 레이아웃의 HTML 에러 페이지로 표시됩니다.
- 상단 메뉴에 현재 위치가 표시됩니다(활성 메뉴 강조).
- favicon을 인라인 SVG(data URI)로 제공해 별도 파일 없이 favicon 404를 방지합니다.
- 모든 목록/조인 테이블은 결과가 0건이면 안내 문구를 표시합니다.
- 포켓몬 상세의 종족값은 수치와 함께 막대 그래프로 표시됩니다.
- 값이 없는 수치(위력/명중/PP/성질/순위 등)는 "-"로 표시됩니다(dash 필터, 빈 문자열 포함).
- 이미지 URL이 깨진 경우 깨진 아이콘 대신 빈 자리 표시로 대체됩니다.
- 기술/특성 상세에도 원본(gamewith.ai) 링크가 표시됩니다.
- 첫 화면 카드에 각 데이터 건수가 표시됩니다.

## 테스트

queries.py의 조회 결과를 검증하는 pytest 테스트가 tests/ 폴더에 있습니다.
...
- 현재 데이터(레귤레이션 M-A/M-B, 시즌 M-3) 기준의 기대값을 하드코딩해 두었습니다.
- CSV를 갱신해 데이터가 바뀌면 테스트 기대값도 함께 갱신해야 합니다.
- 스프링부트(MyBatis) 이관 시, 같은 파라미터 조합으로 SQL 결과를 비교하는 기준으로 사용합니다.
