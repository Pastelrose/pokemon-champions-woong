...
def pokemon_search(q="", types=None, type_mode="and", ability="",
    ...
    sql = ("SELECT p.ID, p.도감번호, p.이름, p.이미지URL, p.타입1, p.타입2, p.HP, p.공격, p.방어, p.특공, p.특방, "
...
def move_list(q="", type_="", category="", sort_cols=None, sort_dirs=None):
    ...
    sql = "SELECT ID, 기술명, 타입, 분류, 위력, 명중, PP, 대상, 효과 FROM moves"
    if where:
        sql += " WHERE " + " AND ".join(where)
    order = []
    for col, d in zip(sort_cols or [], sort_dirs or []):
...
def item_list(q=""):
    if q:
        ...
        return query("SELECT ID, 이름, 이미지URL, 효과, 입수방법 FROM items "
                     "WHERE 이름 ILIKE '%' || ? || '%' OR 효과 ILIKE '%' || ? || '%' "
                     "ORDER BY 이름", [q, q])
    return query("SELECT ID, 이름, 이미지URL, 효과, 입수방법 FROM items ORDER BY 이름")
